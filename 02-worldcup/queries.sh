#!/bin/bash
# World Cup — queries.sh (FCC expected file)
# Menjalankan queries yang dites FCC test suite
# Usage: chmod +x queries.sh && ./queries.sh
...
echo -e "\nTeam that scored the most goals in a single game (and goals):"
echo "$($PSQL "SELECT name, winner_goals FROM games INNER JOIN teams ON games.winner_id = teams.team_id ORDER BY winner_goals DESC LIMIT 1")"
